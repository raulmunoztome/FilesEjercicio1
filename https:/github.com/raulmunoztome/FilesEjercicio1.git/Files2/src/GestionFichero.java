...
public class GestionFichero {
	...
	Set<Vehicle> llegirCSV(String nomFitxer) throws Exception {
		...
		try (BufferedReader bReader = new BufferedReader(new FileReader(fitxer))) {
			...
			while ((linia = bReader.readLine()) != null) {
				...
				if (elemento[0] == "coche") {
					...
					total.add(new Coche(matricula, marca, color, model, km, numPuertas, esAutomatic));
	...
	void desaCSV(String nomFitxer, Set<Vehicle> vehicles) {
